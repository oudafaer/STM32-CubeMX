...
void LCD_WR_CMD(uint8_t dat);//写入一个指令
...
void LCD_ShowIntNum(uint16_t x,uint16_t y,uint16_t num,uint16_t fc,uint16_t bc,uint8_t sizey);    //显示整数变量
//void LCD_ShowIntNum1(uint16_t x,uint16_t y,uint16_t num,uint8_t len,uint16_t fc,uint16_t bc,uint8_t sizey);
...
int LCD_Set_Scroll_Area(uint16_t tfa, uint16_t vsa, uint16_t bta);    //设置滚屏区域
void LCD_Set_Scroll_Start_Address(uint16_t vsp);                      //设置滚屏起始点
void DrawCurve(uint16_t RawValue,uint16_t fc,uint16_t bc);      //画曲线
