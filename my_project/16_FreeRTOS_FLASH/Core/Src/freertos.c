  ...
  for(;;)
  {
	  ...
	  if(key)
	  {
		  if(key==1)
		  {
			...
			data_to_save.int_data[0] =data_read.int_data[0];
			data_to_save.int_data[1] =data_read.int_data[1];
			data_to_save.int_data[2] =data_read.int_data[2];
			data_to_save.int_data[3] =data_read.int_data[3];
			
			
			data_to_save.int_data[0] ++;
			data_to_save.int_data[1] ++;
			data_to_save.int_data[2] ++;
			data_to_save.int_data[3] ++;	
		  ...
		  {
				my_flash_read();//读取的时候就可以把这个注释取消。
				my_printf(1,"%d \r\n",data_read.int_data[2]) ;//这个是要保存的数据
			  led_flag=2;
		  }
	  }
    osDelay(1);
