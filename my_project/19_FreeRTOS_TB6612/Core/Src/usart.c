...
/* USER CODE BEGIN 1 */
void my_printf(int ch,char *format, ...)
{
...
{ 
    ...
    if(huart->Instance == USART1)
    {
			...
			{
				Uart1_Rx_Cnt = 0;
				if(strstr((const char *)RxBuffer,"led1") != NULL)
				{
					//当接收到数据和String1一样时，需要实现的写在这
					led_flag=1;
					LED2_ON();
				}
				if(strstr((const char *)RxBuffer,"led2") != NULL)
				{
					//当接收到数据和String2一样时，需要实现的写在这
					led_flag=2;
					LED1_ON();
				}				
		...
		if(huart->Instance == USART2)
    {
			...
			if((RxBuffer2[Uart1_Rx_Cnt2-1] == 0x0A)&&(RxBuffer2[Uart1_Rx_Cnt2-2] == 0x0D)) //判断结束位
			{
				Uart1_Rx_Cnt2 = 0;
				if(strstr((const char *)RxBuffer2,"oudafa1") != NULL)
				{
					//当接收到数据和String1一样时，需要实现的写在这
	 
				}
				if(strstr((const char *)RxBuffer2,"oudafa2") != NULL)
				{
					//当接收到数据和String2一样时，需要实现的写在这
