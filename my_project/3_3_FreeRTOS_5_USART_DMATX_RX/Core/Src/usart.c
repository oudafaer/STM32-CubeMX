...
uint8_t usart2_rx_buffer[USART_DMA_RX_BUFFER_MAXIMUM]; //串口1的DMA接收缓冲区
uint8_t usart2_tx_buffer[USART_DMA_TX_BUFFER_MAXIMUM]; //串口1的DMA发送缓冲区
uint8_t usart2_rx_flag = 0; //DMA接收成功标志 0，未接收到/1,,接收到等待处理
uint16_t usart2_rx_len = 0; //DMA一次空闲中断接收到的数据长度
uint8_t receive_u2_data[USART_DMA_RX_BUFFER_MAXIMUM];	  //DMA接收数据缓存区
uint8_t USART2_RX_BUF[USART_DMA_RX_BUFFER_MAXIMUM]; //接收缓冲,最大USART_REC_LEN个字节.
uint16_t USART2_RX_STA = 0;			 //接收状态标记
uint8_t aRxBuffer2[EN_USART2_RX];	 //HAL库使用的串口接收缓冲
/* USER CODE END 0 */
...
void printf2(char *format, ...)
{
	va_list args;
	va_start(args, format);
	uint16_t len = vsnprintf((char *)usart2_tx_buffer, sizeof(usart2_tx_buffer), (char *)format, args);
	va_end(args);
	HAL_UART_Transmit(&huart2,(uint8_t *)usart2_tx_buffer,len,1000);
}
...
{	
	...
	{
		memset(receive_u2_data, 0, USART_REC_LEN);//清除上次缓存的数据
		...
		{
			usart2_rx_flag = 1;
			memcpy(receive_u2_data, USART2_RX_BUF, USART2_RX_STA & 0x3fff);
			USART2_RX_STA = 0;
		}		
